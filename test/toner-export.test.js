...
function seedFixture() {
    ...
    const mv = db.prepare(`INSERT INTO stock_movements (toner_type_id, direction, quantity, unit_cost, printer_ip, note, actor, movement_date)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
    // Girişler
    mv.run(1, 'in', 6, 500, null, 'ABC Bilişim', 'admin', '2024-01-10');
    mv.run(2, 'in', 4, 400, null, 'XYZ Ofis', 'admin', '2024-02-01');
    // Çıkışlar (toner değişimi) — aynı yazıcıya iki kez
    mv.run(1, 'out', 1, 500, '10.0.0.5', 'Esra Serpan', 'operator', '2024-03-01');
    mv.run(1, 'out', 1, 500, '10.0.0.5', 'Esra Serpan', 'operator', '2024-04-15');
    mv.run(2, 'out', 2, 400, null, 'Ozan Kansu', 'operator', '2024-05-01');
}
seedFixture();
...
test('Toner_Degisim başlıkları ve satırları doğru', () => {
    ...
    // İlk çıkış: özel konum departman, not alanı "Alan Kişi" olur
...
test('StokGirisCikis yalnızca girişleri firma bilgisiyle listeler', () => {
    ...
    assert.strictEqual(rows.length - 1, 2);
...
test('Uyumluluk Tablosu markayı doğru sütuna yerleştirir', () => {
    ...
    assert.deepStrictEqual(['B3', 'C3', 'D3', 'E3'].map(at), ['HP', 'CANON', 'SAMSUNG', 'BROTHER']);
